 ...
 *     enabled?: bool, // Enable or disable login throttling (for simple single-firewall configuration) // Default: true
 *     max_count_attempts?: int, // Maximum number of login attempts before throttling (for simple single-firewall configuration) // Default: 3
 *     timeout?: int, // Ban period in seconds (for simple single-firewall configuration) // Default: 600
 *     watch_period?: int, // Period in seconds for tracking attempts (for simple single-firewall configuration) // Default: 3600
 *     firewall?: scalar|null, // Firewall name where login_throttling should be applied (for simple single-firewall configuration) // Default: "main"
 *     storage?: "cache"|"database", // Storage backend for login attempts (for simple single-firewall configuration) // Default: "cache"
 *     rate_limiter?: scalar|null, // Custom rate limiter service ID (for simple single-firewall configuration) // Default: null
 *     cache_pool?: scalar|null, // Cache pool to use for storing the limiter state (for simple single-firewall configuration, only used when storage=cache) // Default: "cache.rate_limiter"
 *     lock_factory?: scalar|null, // Lock factory service ID for rate limiter (for simple single-firewall configuration, only used when storage=cache) // Default: null
 *     firewalls?: array<string, array{ // Default: []
 *         enabled?: bool, // Enable or disable login throttling for this firewall // Default: true
 *         max_count_attempts?: int, // Maximum number of login attempts before throttling // Default: 3
 *         timeout?: int, // Ban period in seconds // Default: 600
 *         watch_period?: int, // Period in seconds for tracking attempts // Default: 3600
 *         storage?: "cache"|"database", // Storage backend for login attempts // Default: "cache"
 *         rate_limiter?: scalar|null, // Custom rate limiter service ID (optional). If not provided, Symfony will use default login throttling rate limiter or database rate limiter if storage=database. Use same service ID to share rate limiter across firewalls. // Default: null
 *         cache_pool?: scalar|null, // Cache pool to use for storing the limiter state (only used when storage=cache) // Default: "cache.rate_limiter"
