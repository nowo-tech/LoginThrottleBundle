...
2. **Product behavior** — what **LoginThrottleBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** security engineer, **I want** native Symfony `login_throttling` preconfigured **so that** brute-force login attempts are slowed without custom listeners. |
| US-02 | **As an** integrator, **I want** sensible defaults for max attempts and interval **so that** I enable throttling with minimal YAML. |
| US-03 | **As an** integrator, **I want** documentation aligned with Symfony Security **so that** upgrades across Symfony 7/8 stay predictable. |
| US-04 | **As a** maintainer, **I want** integration tests in CI **so that** firewall + throttling wiring does not regress. |
...
**Goal:** Symfony bundle for login throttling using native Symfony login_throttling feature with pre-configured settings
